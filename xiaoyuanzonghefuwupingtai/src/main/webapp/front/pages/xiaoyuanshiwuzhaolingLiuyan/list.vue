<template>
	...
	<view class="list b-b" v-for="(item, index) in list" :key="index">
		<view class="wrapper" style="margin-left: 25upx;margin-top: 10upx;width: 800rpx;">
			...
			<view class="address-box" style="margin-bottom: 20rpx;width: 600rpx;">
				<text style="margin-left: 30rpx;">我的留言：{{item.xiaoyuanshiwuzhaolingLiuyanText}}</text>
			</view>
        ...
        <view style="float: right;margin-right: 25upx;">
            <text v-if="isAuth('xiaoyuanshiwuzhaolingLiuyan','删除')" style="margin: 0 50upx 0 30rpx;" class="cuIcon-delete" @click.stop="onDeleteTap(item.id)"></text>
        </view>
	...
	<button
            ...
            v-if="isAuth('xiaoyuanshiwuzhaolingLiuyan','新增')" class="add-btn" @click="onAddTap()">新增</button>
...
<script>
	export default {
		...
		methods: {
			//查询条件切换
			queryChange(e) {
				this.searchForm.xiaoyuanshiwuzhaoling_liuyanName = "";
			},
			//类别搜索
