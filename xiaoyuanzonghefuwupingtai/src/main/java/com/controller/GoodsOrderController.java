    ...
    public R info(@PathVariable("id") Long id, HttpServletRequest request){
        ...
        if(goodsOrder !=null){
            ...
            BeanUtils.copyProperties( goodsOrder , view );//把实体数据重构到view中
                ...
                if(goods != null){
                    BeanUtils.copyProperties( goods , view ,new String[]{ "id", "createTime", "insertTime", "updateTime", "yonghuId"});//把级联的数据添加到view中,并排除id和创建时间字段
                    view.setGoodsId(goods.getId());
                    view.setGoodsYonghuId(goods.getYonghuId());
                }
                //级联表
...
//                            goodsOrderEntity.setGoodsOrderText(data.get(0));                    //订单备注 要改的
    ...
    */
    ...
    public R add(@RequestBody GoodsOrderEntity goodsOrder, HttpServletRequest request){
        logger.debug("add方法:,,Controller:{},,goodsOrder:{}",this.getClass().getName(),goodsOrder.toString());
            ...
            if((goodsEntity.getGoodsKucunNumber() -1)<0){
            ...
            goodsOrder.setGoodsOrderTruePrice(goodsEntity.getGoodsNewMoney()*1); //设置实付价格
            ...
            goodsOrder.setInsertTime(new Date());
            goodsOrder.setCreateTime(new Date());
                goodsEntity.setGoodsKucunNumber( goodsEntity.getGoodsKucunNumber() -1);
